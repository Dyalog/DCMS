	...
	stage('Deploying with Rancher') {
        withCredentials([
            ...
            if (env.BRANCH_NAME.contains('master')) {
                ...
                sh '/usr/local/bin/rancher-compose -f ./docker-compose.yml --access-key $ACCESSKEY --secret-key $SECRETKEY --url http://rancher.dyalog.com:8080/v2-beta/projects/1a5/stacks/1st60 -p TryAPL up --force-upgrade --confirm-upgrade --pull -d'
            }
        }
    }
}