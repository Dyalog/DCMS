...
Error-handling behaviour is governed by three **independent** variables in `#.GLOBAL`, set via `DCMS.SetDebug`, which also propagates them to the Stark router (`router.Debug`) and Jarvis (`jarvis.Debug`):
